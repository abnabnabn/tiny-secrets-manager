...
func (s *Server) handlePutSettings(w http.ResponseWriter, r *http.Request) {
	...
	// Validate inputs before saving any of them
	for k, v := range req {
		switch k {
		case "backup_target":
			if strings.HasPrefix(v, "-") {
				s.respondError(w, http.StatusBadRequest, "backup_target cannot start with a dash")
				return
			}
		case "backup_interval_mins":
			parsed, err := strconv.Atoi(v)
			if err != nil || parsed < 1 {
				s.respondError(w, http.StatusBadRequest, "backup_interval_mins must be an integer >= 1")
				return
			}
		case "backup_retention_all_days":
			parsed, err := strconv.Atoi(v)
			if err != nil || parsed < 0 {
				s.respondError(w, http.StatusBadRequest, "backup_retention_all_days must be an integer >= 0")
				return
			}
		case "backup_retention_daily_days":
			parsed, err := strconv.Atoi(v)
			if err != nil || parsed < 0 {
				s.respondError(w, http.StatusBadRequest, "backup_retention_daily_days must be an integer >= 0")
				return
			}
