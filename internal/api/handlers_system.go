...
func (s *Server) handlePutSettings(w http.ResponseWriter, r *http.Request) {
	...
	// Strictly validate incoming settings keys and values at the input boundary
	for k, v := range req {
		switch k {
		case "backup_target":
		...
		case "backup_interval_mins":
			...
			if err != nil || val < 1 {
				s.respondError(w, http.StatusBadRequest, "invalid backup_interval_mins: must be integer >= 1")
				return
			}
		case "backup_retention_all_days":
			val, err := strconv.Atoi(v)
			if err != nil || val < 0 {
				s.respondError(w, http.StatusBadRequest, "invalid backup_retention_all_days: must be integer >= 0")
				return
			}
		case "backup_retention_daily_days":
			val, err := strconv.Atoi(v)
			if err != nil || val < 0 {
				s.respondError(w, http.StatusBadRequest, "invalid backup_retention_daily_days: must be integer >= 0")
				return
			}
